...
/////////////////////////////////////////////////////////////////////////////
//Default initializer
- (instancetype)initWithURL:(NSURL *)url protocols:(NSArray*)protocols queue:(dispatch_queue_t)customQueue
{
    if(self = [super init])
    {
...
- (void)connect
{
    ...
    //everything is on a background thread.全局并行队列
...
-(void)dequeueWrite:(NSData*)data withCode:(JFROpCode)code
{
    ...
    if(!self.writeQueue)
    {
        self.writeQueue = [[NSOperationQueue alloc] init];//.创建一个其他队列
        //  maxConcurrentOperationCount 最大并发数，用来设置最多可以让多少个任务同时执行。当你把它设置为 1 的时候，他不就是串行了嘛
